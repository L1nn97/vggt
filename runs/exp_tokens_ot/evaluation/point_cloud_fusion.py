# -*- coding: utf-8 -*-
# @Description: Point cloud fusion strategy for DTU dataset based on Open3D Library. Copied from 
...
def load_data(root_path, depth_path, scene_name, thresh):
    ...
    if not os.path.exists(scene_dir):
        print(f"Scene directory {scene_dir} does not exist, skipping...")
...
def prepare_proj_mats(extrinsics, intrinsics):
    proj_mats = []
    for i in range(extrinsics.shape[0]):
        proj_mat = np.eye(4, dtype=np.float32)
        proj_mat[:3, :4] = np.dot(intrinsics[i][:3, :3], extrinsics[i][:3, :4])
        proj_mats.append(proj_mat)
    return torch.from_numpy(np.array(proj_mats)).float()

def fusion_point_cloud(pred_depths, extrinsics, intrinsics, rgbs, dist_thresh, num_consist, scale_factor=1.0):
    proj_mats = prepare_proj_mats(extrinsics, intrinsics)
    depths = pred_depths.squeeze()
    N, H, W = depths.shape[0], depths.shape[1], depths.shape[2]
    points = []

    for i in range(N):
        ref_depth = depths[i].reshape(1, 1, H, W).float()
        src_depths = depths.reshape(N, 1, H, W).float()
        ref_proj = proj_mats[i].reshape(1, 4, 4).float()
        src_projs = proj_mats.reshape(N, 4, 4).float()

        _, aligned_pred_points_from_depth, dist = filter_depth(
            ref_depth=ref_depth,
        ...
        masked_pc = aligned_pred_points_from_depth * masks
        ...
        final_pc_with_color = extract_points(avg_points, final_mask, rgbs[i].permute(1, 2, 0))
        points.append(final_pc_with_color)
    
    print(f"points shape: {len(points)}")
    points = np.concatenate(points, axis=0)
    points[:, :3] = points[:, :3] * scale_factor
    return points[:, :3], points[:, 3:].astype(np.float32)

def fusion_point_cloud1(pred_depths, extrinsics, intrinsics, rgbs, dist_thresh, num_consist, scale_factor=1.0):
    proj_mats = prepare_proj_mats(extrinsics, intrinsics)
    depths = pred_depths.squeeze()
    N, H, W = depths.shape[0], depths.shape[1], depths.shape[2]
    
    # Pre-convert to tensors and ensure correct shape and device
    if isinstance(proj_mats, np.ndarray):
    ...
    # Get device from depths (assuming they're already on the correct device)
    device = depths.device if hasattr(depths, 'device') else torch.device('cpu')
    proj_mats = proj_mats.to(device)
    depths = depths.to(device)
    
    # Pre-reshape depths once
    depths_reshaped = depths.view(N, 1, H, W)
    proj_mats_reshaped = proj_mats.view(N, 4, 4)
    
    # Pre-generate all point clouds for all views (batch operation)
    all_point_clouds = generate_points_from_depth(depths_reshaped, proj_mats_reshaped)  # (N, 3, H, W)
    
    points = []
    
    for i in range(N):
        ref_depth = depths_reshaped[i:i+1]  # (1, 1, H, W)
        ref_proj = proj_mats_reshaped[i:i+1]  # (1, 4, 4)
        ref_pc = all_point_clouds[i:i+1]  # (1, 3, H, W)
        
        # Batch warp all source point clouds to reference view
        aligned_pcs = homo_warping(
            all_point_clouds,  # (N, 3, H, W)
            proj_mats_reshaped,  # (N, 4, 4)
            ref_proj,  # (1, 4, 4)
            ref_depth  # (1, 1, H, W)
        )  # (N, 3, H, W)
        ...
        y_2 = (ref_pc[:, 1] - aligned_pcs[:, 1])**2
        z_2 = (ref_pc[:, 2] - aligned_pcs[:, 2])**2
        dist = torch.sqrt(x_2 + y_2 + z_2).unsqueeze(1)  # (N, 1, H, W)
        
        # Apply distance threshold
        masks = (dist < dist_thresh).float()
        masked_pc = aligned_pcs * masks
        pc_buff = masked_pc.sum(dim=0, keepdim=False)  # (3, H, W)
        val_cnt = masks.sum(dim=0, keepdim=False)  # (1, H, W)
        
        # Avoid division by zero
        val_cnt = torch.clamp(val_cnt, min=1e-8)
        final_mask = (val_cnt.squeeze(0) >= num_consist)  # (H, W)
        avg_points = torch.div(pc_buff, val_cnt).permute(1, 2, 0)  # (H, W, 3)
        
        # Extract points with color
        final_pc_with_color = extract_points(avg_points, final_mask, rgbs[i].permute(1, 2, 0))
        points.append(final_pc_with_color)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_list', type=str, default='/home/jianyuan/src/eval_dtu/GeoMVSNet/datasets/lists/dtu/test.txt')
    ...
    parser.add_argument('--color_thresh', type=float, default=0.10,
                        help='L2 RGB distance in [0‑1] space that defines color consistency')
    parser.add_argument('--use_color', action='store_true', default=False,
                        help='Enable color consistency checking')
    ...
    def open3d_filter():
        with torch.no_grad():
            ...
            for i, scene in enumerate(all_scenes):
                ...
                # Check if the scene directory exists
                scene_dir = f"{args.depth_path}/{scene}"
                if not os.path.exists(scene_dir):
                    print(f"Scene directory {scene_dir} does not exist, skipping...")
                    continue
                ...
                for i in range(tot_frame):
                    ...
                    while True:
                        ...
                        depth_mask = (dist < args.dist_thresh).float()
                        ...
                        # ---- color part (optional) ----
                        if args.use_color:
                            ref_rgb = rgbs[i:i+1]                                       # (1,3,H,W)
                            src_rgbs = rgbs[j:min(j+batch_size, tot_frame)]             # (B,3,H,W)

                            warped_rgbs = warp_colors(src_rgbs,                         # (B,3,H,W)
                                                    projs[j:min(j+batch_size, tot_frame)],
                                                    projs[i:i+1],
                                                    depths[i:i+1])

                            # Euclidean distance in RGB space (already 0‑1 scale)
                            color_diff = torch.sqrt(((warped_rgbs - ref_rgb)**2).sum(dim=1, keepdim=True))
                            color_mask = (color_diff < args.color_thresh).float()

                            # Combine depth and color masks
                            masks = depth_mask * color_mask        # logical AND
                ...
                ply_id = int(scene[4:])
                write_ply('{}/{:03d}.ply'.format(args.ply_path, ply_id), np.concatenate(points, axis=0))
                del points, depths, rgbs, projs

                gc.collect()

                print('Save {}/{:03d}.ply successful.'.format(args.ply_path, ply_id))
