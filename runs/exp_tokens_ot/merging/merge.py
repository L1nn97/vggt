"""
FastVGGT's token merging function.
"""
    ...
    with torch.no_grad():
        ...
        if enable_protection:
            ...
            protected_indices = torch.arange(0, N, step, device=metric.device)[
                :num_protected # 这里用 :num_protected是边界情况处理, 如果N不能被整除...
            ]
        else:
        ...
        if num_imgs > 1:
            ...
            """
            上面这几行是用来mask掉camera和register token的
            """
            ...
            if no_rand:
                ...
                """
                如果 no_rand, 则先把所有的image patch tokens都标志为0, 即src tokens
                """
            else:
                ...
                """
                这里是如果!no_rand的情况下, 则在每个图像的sx*sy块中, 随机选一个为dst, 其他的都表示为src
                """
        ...
        a_idx_orig = rand_idx[:, num_dst_orig:, :] # a_idx_orig是src tokens的索引
        b_idx_orig = rand_idx[:, :num_dst_orig, :] # b_idx_orig是dst tokens的索引
        a_idx = a_idx_orig
        b_idx = b_idx_orig
        ...
        r = min(a.shape[1], r) # r 是传入的要merge的tokens数量: r = int(x.shape[1] * self.merge_ratio)
        num_src_actual = a.shape[1]
        chunk_size = min(5000, num_src_actual) # 用于内存优化

        node_max = torch.empty(B, num_src_actual, device=a.device, dtype=a.dtype) # 每个 src token 的最大相似度值，形状 (B, num_src_actual)
        node_idx = torch.empty(B, num_src_actual, device=a.device, dtype=torch.long) # 每个 src token 对应的最相似 dst token 的索引，形状 (B, num_src_actual)

        b_transposed = b.transpose(-1, -2) # 将 b 从 (B, num_dst, C) 转置为 (B, C, num_dst)
        node_max, node_idx = fast_similarity_chunks(a, b_transposed, chunk_size)
        edge_idx = node_max.argsort(dim=-1, descending=True)[..., None] # 用于存储每个src token的相似度最大值的索引
        ...
        if enable_protection:
            ...
            unm_idx = edge_idx[..., r:, :] # 这些是未merge的src tokens的索引, 
            src_idx = edge_idx[..., :r, :] # 这些是merge的src tokens的索引, 对应相似度高的前面的tokens
            r_actual = r
