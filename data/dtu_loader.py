...
def default_depth_loader(depth_paths: List[str], device: Optional[str] = None, target_size: Optional[Tuple[int, int]] = None):
    ...
    if load_and_preprocess_images is not None and torch is not None and target_size is None:
        # 如果不需要缩放，直接使用VGGT的预处理
        depths = load_and_preprocess_images(depth_paths)
        if device:
...
class DTUScanLoader:
    ...
    def __init__(
        ...
        # 计算实际使用的尺寸（如果指定了target_size）
        if self.target_size is not None:
            self.W, self.H = self.target_size
            self.scale_x = self.W / self.W_orig
            self.scale_y = self.H / self.H_orig
        else:
    ...
    def load_images(self):
        print(f"Loading images from {self.image_paths}")
        return default_image_loader(self.image_paths, self.device, self.target_size)

    def load_masks(self):
        print(f"Loading masks from {self.mask_paths}")
        return default_mask_loader(self.mask_paths, self.device, self.target_size)

    def load_depths(self):
        print(f"Loading depths from {self.depth_paths}")
        return default_depth_loader(self.depth_paths, self.device, self.target_size)
...
if __name__ == "__main__":
    ...
    loader = DTUScanLoader(dtu_root, scan_id, num_views=4, step=1, device=None, target_size=target_size)
